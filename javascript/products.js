  ...
  filtered.forEach(p => {
    ...
    card.innerHTML = `
      <div class="relative group">
        <img src="${p.image}" alt="${p.name}" class="w-full h-52 object-cover group-hover:scale-105 transition duration-300" />
      ...
      <div class="p-3 text-sm">
        ...
        <div class="flex gap-1">
          <button class="checkoutBtn orderSummary bg-gray-600 text-white w-3/4 px-6 py-2 rounded hover:bg-gray-700 flex justify-around items-center gap-2">
            <span>Check Out</span>
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
              <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3" />
              </svg>
          </button>
          <button class="add-to-cart bg-gray-900 text-white w-1/4 flex justify-center py-2 rounded hover:bg-gray-800"
            data-name="${p.name}"
            data-price="${p.price}"
            data-img="${p.image}">
